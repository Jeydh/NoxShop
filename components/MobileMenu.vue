<template>
  <Teleport to="body">
    ...
    <Transition name="mobile-overlay">
      <div v-if="open" class="fixed inset-0 z-[80] bg-black/60 backdrop-blur-sm" aria-hidden="true" @click="close" />
    </Transition>

    <!-- Drawer -->
    <Transition name="mobile-drawer">
      <aside v-if="open" class="fixed inset-y-0 right-0 z-[90] flex w-[85vw] max-w-sm flex-col bg-slate-950 shadow-2xl"
        ...
        <div class="flex h-20 shrink-0 items-center justify-between border-b border-white/10 px-5">
          ...
          <button type="button"
            ...
            aria-label="Fermer le menu" @click="close">
            ×
          </button>
        </div>
        ...
        <nav class="flex-1 overflow-y-auto p-4">
          <div class="space-y-2">
            <NuxtLink v-for="item in items" :key="item.to" :to="item.to"
              class="group flex items-center gap-3 rounded-2xl px-4 py-3.5 transition-all duration-200" :class="isActive(item.to)
                  ? 'bg-violet-500/15 text-white'
                  : 'text-slate-400 hover:bg-white/5 hover:text-white'
                " @click="close">
              <span class="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl transition-all duration-200"
              ...
              <div class="min-w-0 flex-1">
                ...
                <div v-if="item.description" class="mt-0.5 truncate text-xs text-slate-500">
                  {{ item.description }}
                </div>
              </div>
