<template>
  <div class="min-h-screen bg-nox-950 text-slate-100">
    <header class="sticky top-0 z-40 border-b border-white/5 bg-nox-950/85 backdrop-blur-xl">
      <div class="mx-auto flex max-w-7xl items-center justify-between px-4 py-4 lg:px-8">
        ...
        <div class="flex items-center gap-3">
          ...
          <span :class="[
            ...
            <span class="mr-2 inline-block h-1.5 w-1.5 rounded-full bg-current" />
          ...
          <NuxtLink to="/cart"
            ...
            <Transition name="pop">
              <span v-if="cart.count" :key="cart.count"
          ...
          <button type="button"
            ...
            :aria-expanded="mobileMenuOpen" aria-label="Ouvrir le menu" @click="mobileMenuOpen = true">
            ☰
          </button>
        </div>
      </div>
    ...
    <!-- Menu mobile -->
    <MobileMenu :open="mobileMenuOpen" :items="mobileItems" title="NOX" eyebrow="Night Shop"
